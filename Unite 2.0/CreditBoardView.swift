...
struct LectureforCB: Identifiable {
    var id: Int
    var name: String
    var category: String
...
struct CreditBoardView: View {
    ...
    var lectures = [
        LectureforCB(id: 0, name: "初年次ゼミナール", category: "基礎科目", subcategory: "初年次ゼミナール文科"),
        LectureforCB(id: 1, name: "情報", category: "基礎科目", subcategory: "初年次ゼミナール文科"),
        LectureforCB(id: 2, name: "数学I", category: "基礎科目", subcategory: "社会科学"),
        LectureforCB(id: 3, name: "フランス語一列", category: "総合科目", subcategory: "初年次ゼミナール文科"),
        LectureforCB(id: 4, name: "なんとか展開科目", category: "展開科目", subcategory: "宇宙科学"),
        LectureforCB(id: 5, name: "なんとかゼミナール", category: "主題科目", subcategory: "宇宙科学")
    ]
    
    let selectedLectureid = [1, 3]
    ...
    var selectedLecture = lectures.filter { x in
         return selectedLectureid.contains(x.id)
    }


    
    func yourFunction() {
        for id in selectedLectureid {
            if let lecture = lectures.first(where: { $0.id == id }) {
                selectedLecture.append(leccture)
            }
        }
    }
    */
    ...
    var categorizedLectures: [String: [LectureforCB]] {
            Dictionary(grouping: lectures, by: { $0.category })
    }
    
    var body: some View {
        NavigationView {
            List {
                Section(header: Text("基礎科目").font(.title3).foregroundColor(.accentOrange))
                {
                    HStack {
                        Text("講義名")
                    ...
                    ForEach(categorizedLectures["基礎科目"]!) { lecture in
                        HStack {
                            Text(lecture.name)
                            Spacer()
                            Text("2")
                ...
                Section(header: Text("総合科目").font(.title3).foregroundColor(.accentOrange))
                {
                    HStack {
                        Text("講義名")
                            .foregroundColor(Color.gray)
                    ...
                    ForEach(categorizedLectures["総合科目"]!) { lecture in
                        HStack {
                            Text(lecture.name)
                            Spacer()
                            Text("2")
                ...
                Section(header: Text("主題科目").font(.title3).foregroundColor(.accentOrange))
                {
                    HStack {
                        Text("講義名")
                            .foregroundColor(Color.gray)
                    ...
                    ForEach(categorizedLectures["主題科目"]!) { lecture in
                        HStack {
                            Text(lecture.name)
                            Spacer()
                            Text("2")
                ...
                Section(header: Text("展開科目").font(.title3).foregroundColor(.accentOrange))
                {
                    HStack {
                        Text("講義名")
                            .foregroundColor(Color.gray)
                    ...
                    ForEach(categorizedLectures["展開科目"]!) { lecture in
                        HStack {
                            Text(lecture.name)
                            Spacer()
                            Text("2")
            ...
            .navigationBarTitle("単位表")
        }
    }
}

struct CreditBoardView_Previews: PreviewProvider {
